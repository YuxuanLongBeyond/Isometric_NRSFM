...
lam = 0.;
N = size(a, 1) + 1;
% reg_20 = lam * sum(a .^ 2 + c .^ 2) + lam; 
% reg_11 = lam * sum(2 * a .* b + 2 * c .* d); 
% reg_10 = lam * sum(2 * a .* t1 + 2 * c .* t2);
% reg_02 = lam * sum(b .^ 2 + d .^ 2) + lam; 
% reg_01 = lam * sum(2 * b .* t1 + 2 * d .* t2); 
% reg_00 = lam * sum(t1 .^ 2 + t2 .^ 2);
reg_20 = lam;
reg_11 = 0;
reg_10 = 0;
reg_02 = lam;
...
eq = [sum(eq1_60+eq2_60);sum(eq1_51+eq2_51);sum(eq1_42+eq2_42);sum(eq1_33+eq2_33);sum(eq1_24+eq2_24);sum(eq1_15+eq2_15);sum(eq1_06+eq2_06);...
      ...
      sum(eq1_20+eq2_20) + reg_20;sum(eq1_11+eq2_11) + reg_11;sum(eq1_02+eq2_02) + reg_02;...
      sum(eq1_10+eq2_10) + reg_10;sum(eq1_01+eq2_01) + reg_01;sum(eq1_00+eq2_00) + reg_00];
