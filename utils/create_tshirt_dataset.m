function [Pgth,Ngth,q_n,I1u,I1v,I2u,I2v,J21a,J21b,J21c,J21d,J12a,J12b,J12c,J12d,H21uua,H21uub,H21uva,H21uvb,H21vva,H21vvb] = create_tshirt_dataset(idx,n, scene, par)

for i = 1:n
    P2_n{i} = scene.Pgth(idx(i)).P; % Ground truth 3D points
    q_n{i} = scene.m(i).m; % Image points corresponding to ground truth
end
...
for i = 2:n
    ...
    ctrlpts = cpts';
   q =  bbs_eval(bbs,ctrlpts,q2(1,:)',q2(2,:)',0,0);
%     error=sqrt(mean((q(1,:)-q1(1,:)).^2+(q(2,:)-q1(2,:)).^2));
%     disp(fprintf('[ETA] Internal Rep error = %f',error)); 
...
%     ctrlpts=ctrlpts';
     qw2{i-1} = bbs_eval(bbs,ctrlpts,q2(1,:)',q2(2,:)',0,0);
    error=sqrt(mean((qw2{i-1}(1,:)-q1(1,:)).^2+(qw2{i-1}(2,:)-q1(2,:)).^2));
