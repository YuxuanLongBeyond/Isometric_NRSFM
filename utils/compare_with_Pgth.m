...
for i = 1: size(u,1)
    ...
    umin=min([u(i,idx), q1(1, :)])-t;umax=max([u(i,idx), q1(1, :)])+t;
    vmin=min([v(i,idx), q1(2, :)])-t;vmax=max([v(i,idx), q1(2, :)])+t;
...
%     err_p(i, :) = sqrt(sum((Pg(3*(i-1)+1:3*(i-1)+3,:)-P2(3*(i-1)+1:3*(i-1)+3,:)).^2));
